...
function sumarLista(listaDeNumeros) {
    return listaDeNumeros.reduce((acumulado, numero) => acumulado + numero, 0);
}
...
function enumerarLista(listaDePalabras) {
    ...
    var todasMenosLaUltima = listaDePalabras.slice(0, -1);
    var ultima = listaDePalabras[listaDePalabras.length - 1];
    return todasMenosLaUltima.join(", ") + " y " + ultima + ".";
}
